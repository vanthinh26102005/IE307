...
export default function HomeScreen() {
    ...
    const handleAddToCart = async (product: Product) => {
        ...
        try {
            ...
            const idx = updatedProducts.findIndex((p) => p.productId === product.id);
            if (idx >= 0) {
                updatedProducts[idx] = { ...updatedProducts[idx], quantity: updatedProducts[idx].quantity + 1 };
            } else {
    ...
    const listHeader = useMemo(
        () => (
            <View style={{ gap: 16 }}>
                <Text style={styles.tagline}>Shop for quality, Shop for style</Text>
